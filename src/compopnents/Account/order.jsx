...
export const Orders = () => {
  ...
  // Update token when localStorage changes (e.g., after login/logout)
  useEffect(() => {
  ...
  // Debug logs and toast notifications
  useEffect(() => {
    if (isError) {
      const errorMessage =
    ...
    if (!isLoading && orders.length === 0 && token) {
      toast.info("You haven't placed any orders yet.");
    }
  }, [data, orders, isLoading, isError, error, token]);
